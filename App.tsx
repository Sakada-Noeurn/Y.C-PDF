...
const App: React.FC = () => {
  ...
  const [zippingProjectId, setZippingProjectId] = useState<string | null>(null);
  ...
  const convertProject = async (projectId: string) => {
    ...
    try {
      for (let i = 1; i <= total; i++) {
        // Skip if already converted but allow re-convert if needed (current logic skips)
        const alreadyExists = project.pages.find(pg => pg.pageNumber === i);
        if (alreadyExists && project.status !== ConversionStatus.IDLE) {
          completedCount++;
          continue;
        }
        ...
        setProjects(prev => prev.map(p => {
          if (p.id === projectId) {
            const exists = p.pages.some(pg => pg.pageNumber === i);
            const newPages = exists 
              ? p.pages.map(pg => pg.pageNumber === i ? page : pg)
              : [...p.pages, page].sort((a, b) => a.pageNumber - b.pageNumber);
  ...
  const downloadProjectZip = async (project: PDFProject) => {
    if (project.pages.length === 0 || zippingProjectId) return;
    
    setZippingProjectId(project.id);
    setZipProgress(0);
              ...
              {p.status === ConversionStatus.CONVERTING && (
                <div className="mt-3">
                  ...
                  <div className="flex justify-between mt-1">
                    <span className="text-[8px] font-black text-indigo-600 uppercase tracking-tighter">CONVERTING</span>
                    <span className="text-[8px] font-black text-indigo-600">{p.progress}%</span>
                  </div>
                </div>
              )}
            </button>
              ...
              <div className="flex items-center gap-2">
                 ...
                 {activeProject.pages.length > 0 && (
                   <button 
                     ...
                     disabled={!!zippingProjectId}
                     className={`relative overflow-hidden bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-emerald-200 hover:bg-emerald-700 active:scale-95 transition-all flex items-center gap-2 min-w-[140px] justify-center disabled:opacity-80`}
                   >
                     {zippingProjectId === activeProject.id ? (
                       <>
                         <div className="absolute inset-0 bg-emerald-800 transition-all duration-300" style={{ width: `${zipProgress}%` }} />
                         <span className="relative z-10 flex items-center gap-2">
                           <Loader2 className="w-3.5 h-3.5 animate-spin" />
                           {t.zipping} {zipProgress}%
                         </span>
                       </>
                     ) : (
                       <>
                      ...
                      {activeProject.status === ConversionStatus.CONVERTING && (
                        <div className="mt-4 max-w-xs">
                           <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                              <div className="h-full bg-indigo-600 transition-all duration-300" style={{ width: `${activeProject.progress}%` }} />
                           </div>
                           <div className="flex justify-between mt-1">
                              <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">{t.converting}</span>
                              <span className="text-[10px] font-black text-indigo-600">{activeProject.progress}%</span>
                           </div>
                        </div>
                      )}
                   </div>
